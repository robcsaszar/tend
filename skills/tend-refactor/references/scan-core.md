...
**c. Watch for scoped-style / CSS-scoping risk.** Svelte scopes `<style>` blocks per component at compile time by attaching a scoping class (e.g. `svelte-x1a2b3`) to elements in that component's own markup — a component's styles never reach markup rendered by a child component unless wrapped in `:global(...)`. Extracting markup into a new component therefore removes it from the source component's style scope: descendant and child selectors in the source `<style>` block (`.icon .path`, `> svg`, etc.) silently stop matching, because the moved elements now carry the new component's scoping class instead of the original's. If the markup you're about to extract is targeted by any rule in the source component's `<style>` block, either move those rules into the new component alongside the markup, or treat the extraction as **unsafe until manually verified** in a rendered page — do not force it on the strength of static reading alone.
